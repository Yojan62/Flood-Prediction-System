...
"""
HydroFusion v19 (Merged v15 + v18)

This model combines the superior hybrid architecture of v18
with the *correct, real-data* pipeline from v15.

- CRITICAL FIX: Fetches REAL river_discharge from the Flood API.
- CRITICAL FIX: Removes the "synthetic" target generation.
- CRITICAL FIX: Fixes "too many values to unpack" by correcting prepare_tabular_Xy.
...
- Includes augmentation, SHAP, and all plots.

Usage:
    python flood_model_v19.py --mode train [--quick]
"""
...
# --- FIX for NameError: 'layers' is not defined ---
# We now import 'keras' and 'layers' as objects
try:
...
MODEL_DIR = "ml"
MODEL_FILE = "open_meteo_flood_model_v19.pkl"
FEATURES_JSON = "open_meteo_flood_features_v19.json"
PLOT_BACKTEST = "v19_backtest.png"
PLOT_PEAKS = "v19_backtest_peaks.png"
PLOT_SHAP = "v19_shap_summary.png"
PLOT_OBS_PRED = "v19_obs_vs_pred.png"
PRED_CSV = "v19_test_predictions.csv"
PLOT_RESID_TRAIN = "v19_residual_fit_train.png"
...
FEATURE_LIST = [
    ...
    'rain_x_soil','soil_moisture'
]

# --- LSTM sequence features from v15 ---
SEQ_FEATURES = [
    'discharge_m3_s','dis_rate','rain_roll_3','rain_roll_7','api','rain_lag_1','rain_lag_3','max_hourly_rain_mm', 'soil_moisture'
]
...
def fetch_archive_hourly(lat, lon, start_date, end_date):
    ...
    while year <= end.year:
        ...
        params = {
            ...
            "hourly": "precipitation,soil_moisture_0_1cm,soil_moisture_1_3cm,soil_moisture_3_9cm,et0_fao_evapotranspiration,temperature_2m",
    ...
    rename_map = {
        ...
        'soil_moisture_0_1cm': 'soil__0_1',
        'soil_moisture_1_3cm': 'soil__1_3',
        'soil_moisture_3_9cm': 'soil__3_9',
...
# Train (v19 Pipeline)
# -----------------------
def train_v19(lat=LAT, lon=LON, start=DEFAULT_START, end=DEFAULT_END, lead_days=1, quick=False):
    print(f"HydroFusion v19 — Training {start} → {end} (lat={lat:.2f}, lon={lon:.2f})")
    ...
    if not LGB_AVAILABLE:
        raise RuntimeError("LightGBM is required for v19.")
    ...
    plt.legend(); plt.grid(True); plt.title('v19 Backtest — Observed vs Final Hybrid Prediction')
    ...
    if thr_mask.sum() > 0:
        ...
        plt.legend(); plt.grid(True); plt.title('v19 Peaks')
...
if __name__ == "__main__":
    load_dotenv()
    args = parse_args()
    res = train_v19(args.lat, args.lon, args.start, args.end, lead_days=args.lead, quick=args.quick)
    print("✅ Training v19 complete:", res)